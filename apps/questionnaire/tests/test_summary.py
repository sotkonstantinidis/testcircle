...
from questionnaire.summary_configuration import ConfiguredQuestionnaireSummary
from questionnaire.summary_data_provider import SummaryDataProvider, \
    GlobalValuesMixin


class SummaryConfigurationTest(TestCase):
    """
    Tests for questionnaire.summary_configuration.ConfiguredQuestionnaireSummary
    """

    def setUp(self):
        self.obj = ConfiguredQuestionnaireSummary(
            config=MagicMock(),
            summary_type='full',
...
class SummaryDataProviderTest(TestCase):
    def test_summary_type(self):
        with self.assertRaises(NotImplementedError):
            SummaryDataProvider(config='', questionnaire='')

    @patch.object(ConfiguredQuestionnaireSummary, '__init__')
...
class GlobalValuesMixinTest(TestCase):
    ...
    def setUp(self):
        ...
        """
        class Tmp(GlobalValuesMixin, SummaryDataProvider):
            summary_type = 'type'
            content = ['sample']
